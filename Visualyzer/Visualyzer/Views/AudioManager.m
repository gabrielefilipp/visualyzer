...
-(void)visualyzerBacklightChanged:(NSNotification *)notification {
    for (id<AudioManagerDelegate> observer in _observers) {
        [observer backlightLevelChanged:[notification.object floatValue]];
    }
}
...
- (void)startConnection {
    ...
    for (id<AudioManagerDelegate> observer in _observers) {
        [observer mediaControllerStateChanged:YES];
    }
	...
	while(_isConnected) {
    	...
    	while(_isConnected) {
            ...
            for (id<AudioManagerDelegate> observer in _observers) {
                [observer newAudioDataWasProcessed:_magnitudes withLength:length];
            }
...
-(void)stopConnection {
    ...
    for (id<AudioManagerDelegate> observer in _observers) {
        [observer mediaControllerStateChanged:NO];
    }
}
...
- (int)processAirpodsAudio:(float*)buffer {
	...
	// Just for now
	float scale = _scalingFactor * 1.0f;
